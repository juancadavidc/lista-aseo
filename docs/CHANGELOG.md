...
### 2026-08-30 — Fix: login con Google roto tras subir a better-auth 1.7 (`internal_server_error`)
- **Síntoma** — al volver de Google, `/api/auth/callback/google` respondía 302 hacia `/api/auth/error?error=internal_server_error` y la app mostraba la pantalla "Something went wrong". Afectaba a producción (`home-care.stack-less.com`) tanto en login con Google como en el registro por email.
- **Causa raíz** — el bump de dependencias del 2026-08-21 subió `better-auth 1.5.6 → 1.7.1`, y 1.7 agregó la columna **`issuer`** (NOT NULL + índice único `issuer, accountId`) a la tabla `account`; el callback resuelve al dueño de la cuenta con `findAccountOwnerByKey({ issuer, accountId })`. Las tablas de auth se habían creado con el schema de 1.5.x y no se regeneran solas, así que la query moría con `column account.issuer does not exist` y better-auth traducía ese fallo de DB a `internal_server_error`. Reproducido en local contra una base con el schema exacto de 1.5.6: falla tanto `findAccountOwnerByKey` como `signUpEmail`.
- **Solución** — nueva migración incremental `migrateAccountIssuer` (`server/lib/auth-schema-migration.js`), invocada al final de `migrate()` en el arranque del server. Agrega la columna nullable, backfillea el issuer que espera 1.7 según el `providerId` (`credential → local:credential`, `google → https://accounts.google.com`, resto → `local:oauth:<providerId>`), recién ahí la marca `NOT NULL` y crea el índice único. Es idempotente: si la tabla `account` no existe o la columna ya está, no toca nada.
- **Sin pérdida de cuentas** — el backfill conserva el vínculo de las cuentas de Google existentes, así que los usuarios entran con su misma cuenta en vez de chocar contra el error o crear una nueva.
- **Verificación** — contra una base creada con el schema 1.5.6 y datos legacy (una cuenta `google` y una `credential`): la migración corre una sola vez, `findAccountOwnerByKey` devuelve el usuario correcto, `signUpEmail` vuelve a funcionar y el diff de schema de better-auth 1.7.1 queda vacío. Tests del server 80/80 (5 casos nuevos).
- Archivos: `server/lib/auth-schema-migration.js` (nuevo), `server/lib/auth-schema-migration.test.js` (nuevo), `server/index.js`, `docs/TECHNICAL.md`.
...
- **Sin flash en navegacion** — `frontend/src/lib/homeScreen.js` cachea la preferencia en `localStorage` con key `casa-limpia:home-screen:<houseId>`. `HomeRedirect` (en `main.jsx`) lee de localStorage sincronicamente al renderizar `/`: si la preferencia es `'tasks'` muestra `Home` directamente, si es otra hace `<Navigate replace>` instantaneo. `Layout.jsx` re-sincroniza el cache desde el servidor en cada montaje y cuando se dispara `profileUpdated`, asi cross-device queda consistente en la siguiente sesion.
- **UX del selector** — fila de chips siguiendo el patron visual del selector de rol en invitaciones: chip activo en `--moss-400` con texto blanco, inactivos con borde sobre `--surface-elevated`. Optimistic update: el chip cambia inmediato, si el PUT falla se revierte la UI y el toast muestra error. `tap targets >= 44px` cumplido.
- **Backward compatible** — la columna tiene default `'tasks'`, las casas/usuarios existentes mantienen exactamente el mismo comportamiento. El endpoint sigue aceptando llamadas con solo `{avatar, color}` (las usadas por el modal de perfil).
- **Tests** — frontend 155/155, backend 70/70, build de Vite verde (419 kB, 23 kB CSS).
- Archivos: `server/index.js` (migracion + endpoints), `db/init.sql` (schema inicial), `frontend/src/lib/homeScreen.js` (nuevo, helpers + cache), `frontend/src/main.jsx` (componente `HomeRedirect`), `frontend/src/pages/HouseSettings.jsx` (seccion `Pantalla de inicio` + handler optimistico), `frontend/src/components/Layout.jsx` (sync de cache).
